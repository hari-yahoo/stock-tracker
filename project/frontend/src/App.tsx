    ...
    <section className="panel alerts-panel">
      ...
      {alerts.length ? (
        <div className="alert-list">
          {alerts.slice(0, 5).map((alert) => (
            <article
              ...
              key={`${alert.exitPlanId}:${alert.type}`}
            >
        ...
        <div className="quiet-state">
          ...
          <p>
            Active plans will appear here seven days before their target date.
          </p>
        </div>
      )}
    </section>
...
function InvestmentValueChart({
  ...
  const chart = useMemo(() => {
    const points = history
      ...
      .sort(
        (left, right) =>
          new Date(left.asOf).getTime() - new Date(right.asOf).getTime(),
      );
    const values = points
      .flatMap((point) => [point.invested, point.market])
      .filter((value): value is number => value !== null);
    const minValue = values.length ? Math.min(...values) : 0;
    const maxValue = values.length ? Math.max(...values) : 1;
    const span = Math.max(maxValue - minValue, maxValue * 0.05, 1);
    const domainMin = Math.max(0, minValue - span * 0.08);
    ...
    const y = (value: number) =>
      padding.top +
      plotHeight -
      ((value - domainMin) / domainSpan) * plotHeight;
    ...
    const grid = [0, 0.25, 0.5, 0.75, 1].map((ratio) => {
      const value = domainMin + domainSpan * ratio;
  ...
  return (
    <section
      className="panel value-chart-panel"
      aria-label="Invested amount and market value history"
    >
      <div className="panel-heading value-chart-heading">
        ...
        <div className="chart-legend" aria-hidden="true">
          <span>
            <i className="legend-dot legend-dot--invested" /> Invested
          </span>
          <span>
            <i className="legend-dot legend-dot--market" /> Market value
          </span>
        </div>
      </div>
      {chart.points.length ? (
        <div className="value-chart-body">
          <div className="value-chart-summary">
            <div>
              ...
              <strong>
                {formatMoney(latest?.investedAmount ?? null, currency)}
              </strong>
            </div>
            <div>
              <span>Latest market value</span>
              <strong>
                {formatMoney(latest?.marketValue ?? null, currency)}
              </strong>
            </div>
          </div>
          <div className="value-chart-wrap">
            <svg
              ...
              role="img"
              aria-label="Line chart comparing invested amount and market value over time"
            >
              {chart.grid.map((line) => (
                <g key={line.value}>
                  <line
              ...
              <path
                className="chart-line chart-line--invested"
                d={chart.investedPath}
              />
              <path
                className="chart-line chart-line--market"
                d={chart.marketPath}
              />
              {chart.points.map((point, index) => (
                <g key={point.asOf}>
                  {point.invested !== null && (
                    <circle
                      className="chart-point chart-point--invested"
                      cx={chart.x(index)}
                      cy={chart.y(point.invested)}
                      r="3.8"
                    />
                  )}
                  {point.market !== null && (
                    <circle
                      className="chart-point chart-point--market"
                      cx={chart.x(index)}
                      cy={chart.y(point.market)}
                      r="3.8"
                    />
                  )}
                </g>
        ...
        <div className="quiet-state">
          ...
          <p>
            Add a BUY trade and price snapshot to start plotting invested amount
...
function Dashboard({
  ...
  console.log("Dashboard data:", history);
    ...
    <main className="main-content">
      ...
      {data.warnings.length > 0 && (
        <div className="data-warning" role="status">
          {icons.alert}
          <span>
            <strong>Some totals need attention.</strong>{" "}
      ...
      <section className="metrics-grid" aria-label="Portfolio summary">
        <MetricCard
          ...
          value={formatMoney(totals.currentValue, data.reportingCurrency)}
          detail={`${data.summary.holdingCount} holdings across ${data.summary.accountCount} accounts`}
          featured
        />
        <MetricCard
          eyebrow="Unrealized P/L"
          value={formatMoney(totals.unrealizedPnl, data.reportingCurrency)}
          detail={
            totals.unrealizedPnl
              ? `${pnlToneValue === "negative" ? "" : "+"}${formatMoney(totals.unrealizedPnl, data.reportingCurrency)} open gain`
        ...
        <MetricCard
          ...
          value={formatMoney(totals.realizedPnl, data.reportingCurrency)}
          detail="From allocated closed lots"
          tone={pnlTone(totals.realizedPnl)}
        />
        <MetricCard
          eyebrow="Capital at cost"
          value={formatMoney(totals.costBasis, data.reportingCurrency)}
          detail={`${data.summary.openLotCount} open investment lots`}
        />
      </section>
    ...
    <div className="app-shell">
        ...
        <nav aria-label="Primary navigation">
          {navItems.map((item) => (
            <button
              ...
              disabled={!item.view}
              title={item.view ? undefined : "Coming next"}
              onClick={() => item.view && setView(item.view)}
            >
              {item.icon}
              <span>{item.label}</span>
              {item.view === view && <i className="nav-indicator" />}
            </button>
          ))}
        </nav>
      ...
      <div className="workspace" id="top">
        {view === "ai" ? (
          ...
          <main
            className="main-content loading-state"
