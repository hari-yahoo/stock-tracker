...
function ManualTradeForm({
  ...
  const [newInstrumentType, setNewInstrumentType] = useState<"EQUITY" | "ETF">(
    "EQUITY",
  );
      ...
      .catch((requestError: unknown) =>
        setError(
          requestError instanceof Error
            ...
            : "Could not load accounts and instruments.",
        ),
      );
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      let allocations:
        Array<{ openingTradeId: string; quantity: string }> | undefined;
      if (side === "SELL") {
        ...
        const buys = postedTrades
          .filter(
            (trade) =>
              ...
              trade.instrumentId === effectiveInstrumentId,
          )
          .sort(
            (left, right) =>
        ...
        for (const buy of buys) {
          const consumed = buy.openingAllocations.reduce(
          ...
          if (allocated > 0)
            allocations.push({
              ...
              quantity: decimal(allocated),
            });
          remaining -= allocated;
          if (remaining < 0.0000005) break;
        }
        if (remaining >= 0.0000005)
          throw new Error(
            `Sell quantity exceeds available lots by ${decimal(remaining)}`,
          );
      }
  ...
  return (
    <section className="panel manual-trade-panel">
      <div className="panel-heading">
        <div>
          <span className="section-kicker">New ledger entry</span>
      ...
      >
        <label className="filter-field">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              setSide(nextSide);
              if (nextSide === "SELL" && instrumentId === "NEW")
                setInstrumentId("");
            }}
          >
        ...
        <label className="filter-field">
          ...
          >
            ...
            {side === "BUY" && (
              <option value="NEW">＋ Add new NSE instrument</option>
            )}
          </select>
        </label>
        {instrumentId === "NEW" && (
              ...
              <select
                ...
                onChange={(event) =>
                  setNewInstrumentType(
                    event.target.value as typeof newInstrumentType,
                  )
                }
              >
        ...
        <label className="search-field manual-trade-notes">
          <span>Notes</span>
          <input
            value={notes}
        ...
        {side === "SELL" && (
          <p className="manual-trade-help">
            The sell will be allocated automatically against available BUY lots
            using FIFO.
          </p>
        )}
        ...
        <button
          ...
          disabled={busy || !accountId || !instrumentId}
        >
          {busy ? "Saving…" : `Add ${side.toLowerCase()}`}
        </button>
      </form>
    </section>
  );
...
export function TransactionsScreen() {
  ...
  const filteredTrades = useMemo(() => {
    return trades.filter((trade) => {
      if (accountFilter !== "ALL" && trade.account.name !== accountFilter)
        return false;
  ...
  const buyCount = filteredTrades.filter(
    (trade) => trade.side === "BUY",
  ).length;
  const sellCount = filteredTrades.filter(
    (trade) => trade.side === "SELL",
  ).length;
  const allocatedSellCount = filteredTrades.filter(
    (trade) => trade.closingAllocations.length > 0,
  ).length;
    ...
    <main className="main-content transactions-page">
      <header className="page-header">
        <div>
          ...
          <h1>Transactions</h1>
          <p>
            Review posted and voided trades, search execution notes, and inspect
            sell allocations lot by lot.
          </p>
        </div>
        <button
          className="primary-button"
      ...
      <section
        className="holdings-summary transactions-summary"
      ...
      <section className="panel holdings-workspace">
          ...
          <div className="holdings-controls">
            ...
            <label className="filter-field">
              ...
              <select
                value={status}
                onChange={(event) =>
                  handleStatusChange(event.target.value as TradeStatus)
                }
              >
        ...
        {error ? (
          ...
          <div className="empty-state holdings-empty-state">
            ...
            <p>
              Try another status, widen the account filter, or clear the search
              term.
            </p>
          </div>
        ) : (
                ...
                <tbody>
                  {filteredTrades.map((trade) => (
                    <tr
                      key={trade.id}
                      className={
                        effectiveSelectedId === trade.id ? "is-selected" : ""
                      }
                    ...
                    >
                      <td>
                        <div className="asset-cell">
                          <span
                            className={`asset-mark asset-mark--${trade.side.toLowerCase()}`}
                          >
                            {trade.side.slice(0, 1)}
                          </span>
                          <span>
                            <strong>{trade.instrument.symbol}</strong>
                            <small>
                              {trade.account.name} · {trade.instrument.exchange}
                            </small>
                          </span>
                        </div>
                      </td>
                      ...
                      <td>
                        <span
                          className={`trade-badge trade-badge--${trade.status.toLowerCase()}`}
                        >
                          {trade.status}
                        </span>
                        <small
                          className={`cell-subtle cell-subtle--${sideTone(trade.side)}`}
                        >
                          {trade.side}
                        </small>
                      </td>
            ...
            {selectedTrade && (
              <aside className="holding-detail transaction-detail">
                <div className="holding-detail__hero">
                  ...
                  <p>
                    {selectedTrade.account.name} · {selectedTrade.side} ·{" "}
                ...
                <dl className="holding-stat-list">
                  ...
                  <div>
                    ...
                    <dd>
                      {formatMoney(
                        selectedTrade.price,
                        selectedTrade.instrument.quoteCurrency,
                      )}
                    </dd>
                  </div>
                  <div>
                ...
                <div className="holding-lots">
                  <div className="holding-lots__heading">
                    <h4>Allocations</h4>
                    <span>
                      {selectedTrade.closingAllocations.length +
                        selectedTrade.openingAllocations.length}
                    </span>
                  </div>
                  {selectedTrade.closingAllocations.length > 0 ? (
                    ...
                    <article className="lot-card">
                      ...
                      <small>
                        This trade is not currently linked to another lot.
                      </small>
                    </article>
                  )}
                </div>
